...
class Generator {
  ...
  async create() {
    ...
    spawnSync(
      'npm',
      ['install', '--save-dev', 'electron', '@electron-forge/cli'],
      {
        cwd: this.targetDir,
        stdio: 'inherit'
      }
    ...
    fs.readdir(tplUrl, (err, files) => {
      ...
      files.forEach(async (file) => {
        ...
        if (/\.js$/.test(file)) {
          await ejs.renderFile(oldFile, { path: 'core/build' }).then((data) => {
    ...
    switch (template) {
      ...
      default: {
        ...
        const packageJson = path.join(this.targetDir, 'core', 'package.json');
        const packageObj = fs.readJSONSync(packageJson);
        fs.outputJsonSync(
          packageJson,
          Object.assign({ homepage: '.' }, packageObj),
          {
            spaces: 2
          }
        );
